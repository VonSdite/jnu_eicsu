	...
	else
	{
		...
		for(var i=0;i<objGetInfo.arrZXT.length;i++)
		{
			...
			for(var j=0;j<arrDepartName.length;j++)
			{
				...
				var _id = objGetInfo.arrZXT[i].account+"-"+(j+1);
				strInnerHtml+="<input type='checkbox' name='"+_id+"'"+"id='"+_id+"'"+"value='"+(j+1)+"\' ";
		...
		for(var i=0;i<objGetInfo.arrRLGS.length;i++)
		{
			...
			for(var j=0;j<arrDepartName.length;j++)
			{
				...
				var _id = objGetInfo.arrRLGS[i].account + "-" + (j+1);
				strInnerHtml+="<input type='checkbox' name='"+_id+"' id='"+_id+"' value=' "+(j+1)+"'";
				if(objGetInfo.arrRLGS[i].department==null) objGetInfo.arrRLGS[i].department=[];
					
				for(var k=0;k<objGetInfo.arrRLGS[i].department.length;k++)
				{
					if(j==objGetInfo.arrRLGS[i].department[k].num-1)
					{
						strInnerHtml+="checked='checked' ";
					}
				}
		...
		document.getElementById("adminsubmit").onclick=function()
		{
			...
			for(var i=0;i<objGetInfo.arrRLGS.length;i++)
			{
				...
				for(var j=0;j<arrDepartName.length;j++)
				{
					...
					var strid=objGetInfo.arrRLGS[i].account+"-"+(j+1);
					
					var c=document.getElementById(strid);
					
					if(c.checked==true)
					{
						checkedDepartment.push({"num":c.value});
					}
				}
				function classGJBM()
				{
					this.account=objGetInfo.arrRLGS[i].account;
					this.department=checkedDepartment;
				}
            ...
            function checkArrGJBM()
            {
                ...
                {
                    for(var j=0;j<arrGJBM[i]['department'].length;++j)
					{
						arrBM[arrGJBM[i]['department'][j]]++;
					}
                }
				for(var i=0;i<arrBM;++i)
				{
					if(arrBM[i]!=1){//每个部门有且仅有一个跟进干事
						return false;
					}
				}
