...
var arrUserType = new Array("干事级","人力干事级","部长级","主席团级");

//非常不好的职位数组，暂时充数
var arrUserType_post = new Array("干事","副部长","部长","第一副书记兼主席","常务副书记兼副主席","副书记兼常务副主席","副书记兼副主席","秘书长");
...
function ShowPersonalData()
{
    ...
    strPersonalDataHTML =
        ...
        + "<div class=\"work_filed\" id=\"user_info_work_filed\">\n"
            ...
            if(objPersonalData.type==1||objPersonalData.type==2) 
            	strPersonalDataHTML += arrUserType_post[0];
         	else if(objPersonalData.type==4)
         	{
         		if(objPersonalData.name=="陈少锋")
         			strPersonalDataHTML += arrUserType_post[3];
         		else if(objPersonalData.name=="俞静")
         			strPersonalDataHTML += arrUserType_post[4];
         		else if(objPersonalData.name=="朱键楠")
         			strPersonalDataHTML += arrUserType_post[5];
         		else if(objPersonalData.name=="孔智生"||objPersonalData.name=="罗宝敏")
         			strPersonalDataHTML += arrUserType_post[6];
         		else if(objPersonalData.name=="章琬苓")
         			strPersonalDataHTML += arrUserType_post[7];
         	}
         	else if(objPersonalData.type==3)
         	{
         		if(objPersonalData.name=="叶敬南"||objPersonalData.name=="范昊东"||objPersonalData.name=="王毅林"||objPersonalData.name=="张子睿"||objPersonalData.name=="马博"||objPersonalData.name=="黄学林"||objPersonalData.name=="王和钰"||objPersonalData.name=="蔡宗辉"||objPersonalData.name=="陈申"||objPersonalData.name=="黄柠")
         			strPersonalDataHTML += arrUserType_post[2];
         		else
         			strPersonalDataHTML += arrUserType_post[1];
         	}
					...
					+ "<div class=\"main_per_info\">\n"
						+ "<div class=\"account\" id=\"account_info\">\n"
							...
							+ "<div class=\"info_item\" id=\"account_info_detail\"><!--这是要从服务器拿数据的，这里只是例子-->\n"
								...
								+ "</p>\n"
								+ "<p>\n"
									+ arrUserType[objPersonalData.type-1] + "\n"
								+ "</p>\n"
								+ "<p>\n"
                               		+ arrDepartName[objPersonalData.apartment-1] + "\n"
								+ "</p>\n"
			if(objPersonalData.type==1||objPersonalData.type==2) 
            	strChangePersonalDataHTML += "<p>" + arrUserType_post[0] + "&nbsp;</p>\n";
         	else if(objPersonalData.type==4)
         	{
         		// var arrUserType_post = new Array("干事","副部长","部长","第一副书记兼主席","常务副书记兼副主席","副书记兼常务副主席","副书记兼副主席","秘书长");
         		if(objPersonalData.name=="陈少锋")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[3] + "&nbsp;</p>\n";
         		else if(objPersonalData.name=="俞静")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[4] + "&nbsp;</p>\n";
         		else if(objPersonalData.name=="朱键楠")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[5] + "&nbsp;</p>\n";
         		else if(objPersonalData.name=="孔智生"||objPersonalData.name=="罗宝敏")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[6] + "&nbsp;</p>\n";
         		else if(objPersonalData.name=="章琬苓")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[7] + "&nbsp;</p>\n";
         	}
         	else if(objPersonalData.type==3)
         	{
         		if(objPersonalData.name=="叶敬南"||objPersonalData.name=="范昊东"||objPersonalData.name=="王毅林"||objPersonalData.name=="张子睿"||objPersonalData.name=="马博"||objPersonalData.name=="黄学林"||objPersonalData.name=="王和钰"||objPersonalData.name=="蔡宗辉"||objPersonalData.name=="陈申"||objPersonalData.name=="黄柠")
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[2] + "&nbsp;</p>\n";
         		else
         			strChangePersonalDataHTML += "<p>" + arrUserType_post[1] + "&nbsp;</p>\n";
         	}
			strChangePersonalDataHTML += 
							 "</div>\n"
						+ "</div>\n"
...
function ChangePersonalData(objPersonalData)
{
 ...
 //    }
    ...
    $("#per_info_modf_submit").click(function(){
        strError= strULE + strUSE + strUQE + strMUDE + strUEE;
		if("" == strError)
		{	
			console.log("发送数据:");
...
function ShowContactsBooks()
{
	...
	strHTML += "<tfoot>\n"
				...
				+ "</tr>\n"
				+ "<tr>\n"
					+ "<td colspan=\"9\" scope=\"col\">主席团共" + iPRSum + "人，部长级+干事共" + (iRestSum -3) + "人</td>\n"
					// (iRestSum -3) 减3是因为有三个空账号，以后修了bug要注意不减
				+ "</tr>\n"
			+ "</tfoot>\n";
	...
	{	
        console.log(arrObjSortPD[iCount][0]);
		...
		if(arrObjSortPD[iCount].length > 0)
		{
            var strDepart = "<tr class=\"first_line\">\n"
						...
						+ "<td>" 
			if(arrObjSortPD[iCount][0].depart==12)
         		strDepart += arrUserType_post[3];
			else
			{
				if(arrObjSortPD[iCount][0].name=="王晓腾"||arrObjSortPD[iCount][0].name=="王子怡"||arrObjSortPD[iCount][0].name=="何菁菁") strDepart+= arrUserType_post[1];
				else strDepart+=arrUserType_post[2];
			}
            ...
            {
                console.log(arrObjSortPD[iCount][jCount]);
                strPersons += "<tr>\n"
                			+ "<td>"

                if(arrObjSortPD[iCount][jCount].name=="俞静")
         			strPersons += arrUserType_post[4];
         		else if(arrObjSortPD[iCount][jCount].name=="朱键楠")
         			strPersons += arrUserType_post[5];
         		else if(arrObjSortPD[iCount][jCount].name=="孔智生"||arrObjSortPD[iCount][jCount].name=="罗宝敏")
         			strPersons += arrUserType_post[6];
         		else if(arrObjSortPD[iCount][jCount].name=="章琬苓")
         			strPersons += arrUserType_post[7];
         		else if(arrObjSortPD[iCount][jCount].name=="王婧"||arrObjSortPD[iCount][jCount].name=="谢智贤"||arrObjSortPD[iCount][jCount].name=="莫希文"||arrObjSortPD[iCount][jCount].name=="桂春垚"||arrObjSortPD[iCount][jCount].name=="涂真"||arrObjSortPD[iCount][jCount].name=="单嘉栋"||arrObjSortPD[iCount][jCount].name=="陈晓红"||arrObjSortPD[iCount][jCount].name=="姚洁"||arrObjSortPD[iCount][jCount].name=="郭舒馨"||arrObjSortPD[iCount][jCount].name=="洪超然"||arrObjSortPD[iCount][jCount].name=="梁慧蓝"||arrObjSortPD[iCount][jCount].name=="李赞"||arrObjSortPD[iCount][jCount].name=="何菁菁"||arrObjSortPD[iCount][jCount].name=="查宽"||arrObjSortPD[iCount][jCount].name=="袁紫薇"||arrObjSortPD[iCount][jCount].name=="黎新智"||arrObjSortPD[iCount][jCount].name=="黄浩贤"||arrObjSortPD[iCount][jCount].name=="陈蓉"||arrObjSortPD[iCount][jCount].name=="李立楷"||arrObjSortPD[iCount][jCount].name=="王子怡"||arrObjSortPD[iCount][jCount].name=="王晓腾")
         			strPersons += arrUserType_post[1];
         		else if(arrObjSortPD[iCount][jCount].name=="王和钰"||arrObjSortPD[iCount][jCount].name=="陈申"||arrObjSortPD[iCount][jCount].name=="黄柠")
         			strPersons += arrUserType_post[2];
         		else strPersons += arrUserType_post[0];
	...
	function UnitTable(tableId)
	{
		...
		{
            var cell_type = objTable.rows[startRow].cells[0].innerHTML;
			if( (cell_type == "干事"||cell_type == "人力干事" || cell_type == ' '||cell_type =="副部长") 
            &&objTable.rows[startRow].cells[0].innerHTML == objTable.rows[i+1].cells[0].innerHTML)
			{
                ...
                var united = objTable.rows[startRow].cells[0].innerHTML;
                if(united == ' ' || !united){
                    objTable.rows[startRow].cells[0].innerHTML = ' ';
                }
			}
			else
			{
