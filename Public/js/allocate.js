...
function showChaXunKeBiao()
{
    ...
    function getQueryDate(){
        ...
        if(tips_qMonth<10) tips_qMonth = "0"+tips_qMonth;
        if(tips_qDay<10) tips_qDay = "0"+tips_qDay;
        date_str = [tips_qYear,tips_qMonth,tips_qDay].join('-')+"T00:00:00Z";
        ret = new Date(date_str);
        return ret;
    }
    //计算周数
    function calWeekParity(start, target){//接受两个Date对象, 从start开始, 计算target相对start为第几周
        var start_date = moment(start);
        var end_date = moment(target);
        return end_date.diff(start_date,'weeks');
    }
    // 获取周数
    function getWeekParity(){
        var d0=GLOBAL_SEMESTER_START;//这个定义在html那里, 是个php echo出来的变量, 原始值定义在AllocateAction.class.php
        var d1= getQueryDate();
        return calWeekParity(d0,d1);
    }
    //提示所选日期是单双周,星期几
    $("#shijian_nian,#shijian_yue,#shijian_ri").change(function(){
        ...
        var oddWeek = ["单","双"];
        var str_tips = "第"+ (w+1) +"周, "+oddWeek[isDanzou]+"周, 星期"+weekDays[moment(d1).days()];
	...
	$("#q_submit").click(function()
	{
		...
		else
		{
			...
			if(arrResponse.length!=0)
			{
				...
				{
                    //先检查一下数据有没有问题
                    if(!arrResponse[i].userName){
                        continue;
                        //可能是删除的时候没删干净, 会有一些奇怪的人被查询出来
                    }
					strQueryReTable += "<tr><td><input type=\"checkbox\" name=\""+arrResponse[i].userID+"\" value=\""+arrResponse[i].userID+"\"/></td>"
									...
									+"<td>"+parseFloat(arrResponse[i].freeTime).toFixed(2) + "</td>"
				...
				$("#al_submit").click(function()
				{
					...
					{
                        var checked = false;
                        try{checked=objAllocFormList.elements[i].checked;}catch(e){}
						if(true == checked)
						{
